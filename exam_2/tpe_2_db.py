...
def insert_2_mongo_tpe(L, i, each_tai_url):
    fail_url = []
    try:
        # headers = {
...
df = pd.read_csv("./tpe_url/591_url_%s.csv" % file_num)
...
with Manager() as manager:
    ...
    for each_url_list in split_tpe_list:
        ...
        if len(each_url_list) == 1000:
            for i in range(1000):
                p = Process(target=insert_2_mongo_tpe, args=(L, i, each_url_list[i]))  # Passing the list
                p.start()
                processes.append(p)
            for p in processes:
                p.join()
    ...
    df = pd.read_csv("./tpe_final.csv")
    # url_dict = {"rent_data": L}
    url_dict = L
    df_tmp = pd.DataFrame(list(url_dict))
    df = pd.concat([df, df_tmp], axis=0, ignore_index=True)
    print(df)
    df.to_csv('./tpe_final.csv' , index=False, encoding='utf_8_sig')